...
export class CartComponent implements OnInit {
  ...
  enviarWP() {
    let mensaje = '';
    this.products.forEach(function (item, indice, array) {
      mensaje += item.quantity + '%20' + item.product.name  + '%20' + (item.quantity * item.product.parsedPrice) + '%20%20' ;
    });
    window.location.href = `https://api.whatsapp.com/send?phone=5492994209101&text=` + (mensaje + 'Gracias');
  }
